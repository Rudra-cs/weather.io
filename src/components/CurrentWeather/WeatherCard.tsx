...
const WeatherCard = (): JSX.Element => {
  ...
  const time: string = useConvertTime(weatherdata?.dt);

  const theme = useRecoilValue(choosenTheme);
  const svgCol = theme === "light" ? "#0F172A" : "#FFFFFF";

  return (
    <div className="flex flex-col border-solid border-[1px] border-slate-900 rounded-lg bg-gray-100 dark:bg-slate-900 h-full py-8 px-6 ">
      <div className="">
        <div className="flex flex-row my-[14px] mx-[16px]">
          <img
            className="h-[96px] w-[96px] dark:invert"
            src={`./animated/${weatherdata?.weather[0].icon}.svg`}
            alt="weather"
          />
        </div>
        <p className="my-[14px] mx-[16px] font-medium text-4xl text-slate-900 dark:text-white">
          {Math.floor(weatherdata?.main.temp)}°C
        </p>
        <p className="tracking-wide my-[14px] mx-[16px] capitalize  text-base text-slate-900 dark:text-gray-500">
        ...
        <div className="flex flex-row my-[14px] mx-[16px]">
          ...
          <p className=" text-slate-900 capitalize text-base tracking-wide dark:text-gray-500">
        ...
        <div className="flex flex-row mx-[16px] ">
          ...
          <p className=" tracking-wide capitalize text-base text-slate-900 dark:text-gray-500">
            {time}
          </p>
        </div>
      </div>
